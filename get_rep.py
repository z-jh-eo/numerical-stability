...
def batch_proc(batch_paths: pd.Series) -> tuple[list[torch.Tensor], list[int]]:
    audio_list = []
    sample_nbs = []
    ...
    for i, L in enumerate(frame_lengths):
        L = int(L.item())
        trimmed.append(out.last_hidden_state[i, :L].cpu())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--metadata-path", "-m", default="./rec_metadata.tsv")
    parser.add_argument("--output-path", "-o", default="./word_reps.pt")
    ...
    audio = pd.Series(df["recording"].unique())
    ...
    for paths in batch_iter(audio):
        frame_reps, sample_nbs = batch_proc(paths)
        all_frame_reps.extend(frame_reps)
        all_sample_nbs.extend(sample_nbs)

    audio_df = pd.DataFrame({"recording": audio.values,
                             "frame_reps": all_frame_reps,
                             "n_samples": all_sample_nbs})

    df_merged = df.merge(audio_df, how="left", on="recording")

    word_reps: list[torch.Tensor] = []
    for row in tqdm(df_merged.itertuples(index=False), total=df_merged.shape[0], desc="Aggregating word representations..."):
        # for row in df_merged.itertuples(index=False):
        rep = mean_pool_word(row.frame_reps, row.start_time,
                             row.end_time, row.n_samples)
        word_reps.append(rep)

    df_merged.drop(["frame_reps", "n_samples"], axis=1)

    df_merged.to_csv("metadata.tsv", sep="\t")
    torch.save(torch.stack(word_reps), args.output_path)
